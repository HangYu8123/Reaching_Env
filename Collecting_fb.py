...
def read( filename="feedback", obs_size = 4, action_size = 1):
    feedbacks = []
...
env = reach_env.Reaching(episode_time=10)
...
def dqn(n_episodes=200, max_t=1000, eps_start=0.4, eps_end=0.01, wt_start = 1, wt_end = 0.1, 
    ...
    save_fb(feedback)
    action_list = []
    for i_episode in range(n_episodes/2):
        state = env.reset()
        score = 0
        ...
        for t in range(max_t):
            ...
            #eps = 0.1 
            prob  = agent.act(state, 0.3)#fb_agent.act(state,0) #*  #* weight_by_frame(cnt)
            ...
            score += buffer[-1][2] #
            #time.sleep(1)
            feedback  = input("please evaluate the robot's behavior in a scale of 0 to 9:")
        ...
        save_fb(feedbacks,str(i_episode)+"_fb")
    ...
    for i_episode in range(n_episodes/2):
        ...
        print("***************start a new one ************")
        scores.append(score)
        save_fb(feedbacks, str(i_episode)+"_fb_repeated")
        #print('\rEpisode {}\tAverage Score: {:.2f} \tCrt Score: {:.2f} \tepsilon: {:.4f}'.format(i_episode, np.mean(scores_window),score,eps), end="")
