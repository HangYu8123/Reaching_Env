...
agent = Agent(state_size=4, action_size=5, seed=0, learning_rate=0.01)
...
for i in range(test/2):
    fb.append(fblib.read_fb(str(i)+"_fb"))
    fb_rp.append(fblib.read_fb(str(i)+"_fb_repeated"))

# for i in range(test/2):
#     fb[i] = classifier.STEADY(fb[i])
#     fb_rp[i] = classifier.STEADY(fb_rp[i])
...
for i in range(num_of_eps/2):
    agent.memory.read_plus_fb( fb_rp[i], filename=str(2*i))

for i in range(agent.memory.__len__()):    
    agent.active_learn(10)
