...
Qagent = QL.QLAgent(5,epsilon = 0.2, mini_epsilon = 0.01, decay = 0.999)
...
def dqn(n_episodes=200, max_t=1000, eps_start=0.4, eps_end=0.01, wt_start = 1, wt_end = 0.1, 
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        for t in range(max_t):
            ...
            #eps = 0.1 
            prob = Qagent.action_prob(state)#fb_agent.act(state,0) #*  #* weight_by_frame(cnt)
            ...
            for b in range(len(buffer)):
                Qagent.learning(action,buffer[b][2],buffer[b][0],buffer[b][1])
            #for b in range(len(buffer)):
